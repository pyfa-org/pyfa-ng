...
class StatsPaneItem(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        super(StatsPaneItem, self).__init__(**kwargs)
    ...
    def add_widget(self, widget):
        if self.container is None:
            return super(StatsPaneItem, self).add_widget(widget)
        return self.container.add_widget(widget)

    def remove_widget(self, widget):
        if self.container:
            self.container.remove_widget(widget)
        super(StatsPaneItem, self).remove_widget(widget)
    ...
    def on_touch_down(self, touch):
        ...
        if self.collapse:
            ...
            return super(StatsPaneItem, self).on_touch_down(touch)
...
class StatsPane(Widget):
    ...
    def __init__(self, **kwargs):
        super(StatsPane, self).__init__(**kwargs)
    ...
    def add_widget(self, widget, *largs):
        if not isinstance(widget, StatsPaneItem):
            raise AccordionException('Accordion accept only AccordionItem')

        widget.accordion = self
        ret = super(StatsPane, self).add_widget(widget, *largs)
        return ret
...
if __name__ == '__main__':
    ...
    acc = StatsPane()
    for x in range(10):
        item = StatsPaneItem(title='Title %d' % x)
        if x == 0:
